...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for horizontal, vertical in horizontalVertical:

            newHorizontal = self.zero_loc[0] + horizontal

            newVertical = self.zero_loc[1] + vertical

            if 0 <= newHorizontal < 3 and 0 <= newVertical < 3:

                newNBR = [row[:] for row in self.state]

                newNBR[self.zero_loc[0]][self.zero_loc[1]], newNBR[newHorizontal][newVertical] = newNBR[newHorizontal][newVertical], \
                                                                                         newNBR[self.zero_loc[0]][self.zero_loc[1]]

                nbr_states.append(EightPuzzleState(newNBR, self.goal, self.dist_from_start + 1, self.use_heuristic, [newHorizontal, newVertical]))
    ...
    def compute_heuristic(self):
        ...
        for idxOne in range(3):

            for idxTwo in range(3):

                result = self.state[idxOne][idxTwo]

                if result != 0:

                    resultRow = result // 3

                    resultColumn = result % 3

                    total += manhattan((idxOne, idxTwo), (resultRow, resultColumn))
