...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        nbr_states = [SingleGoalGridState(eachN, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors) for eachN in self.maze_neighbors(*self.state)]
    ...
    def compute_heuristic(self):
        ...
        horizontalOne, verticalOne = self.state

        horizontalTwo, verticalTwo = self.goal[0]

        return abs(horizontalOne - horizontalTwo) + abs(verticalOne - verticalTwo)
    
    # TODO(V): implement this method... should be unchanged from before
    def __lt__(self, other):

        newSelf = self.dist_from_start + self.h

        newOther = other.dist_from_start + other.h
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        nextLoc = set(self.goal)

        if self.state in nextLoc:

            nextLoc.remove(self.state)

        nbr_states = [GridState(eachN, tuple(nextLoc), self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache) for eachN in self.maze_neighbors(*self.state)]
    ...
    def compute_heuristic(self):
        ...
        if len(self.goal) == 1:
            ...
            if self.goal in self.mst_cache:
                ...
                return self.mst_cache[self.goal]

            else:

                mstResult = compute_mst_cost(self.goal, lambda horizontal, vertical: abs(horizontal[0] - vertical[0]) + abs(horizontal[1] - vertical[1]))

                self.mst_cache[self.goal] = mstResult
