...
class NeuralNet(nn.Module):
    ...
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        # Here we are creating a linear layer transforming input of size 'in_size' to a hidden size of 31.
        self.in_size = nn.Linear(in_size, 31)

        # Here we are creating a linear layer transforming the hidden size of 31 to the 'out_size'.
        self.out_size = nn.Linear(31, out_size)
    ...
    def forward(self, x):
        ...
        # Here we are applying input transformation, ReLU activation, then output transformation.
        return self.out_size(F.relu(self.in_size(x)))
...
def fit(train_set, train_labels, dev_set, epochs, batch_size = 100):
    ...
    net = NeuralNet(0.01, nn.CrossEntropyLoss(), train_set.size(dim = 1), 4)
    ...
    for each in range(epochs):
        ...
        totalLoss = sum(net.step(eachOne['features'], eachOne['labels']) for eachOne in DataLoader(get_dataset_from_arrays(normalizedTrainSet, train_labels), batch_size, shuffle = False))
